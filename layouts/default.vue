<template>
  <v-app v-cloak class="hidden">
    <v-app-bar fixed flat>
      ...
      <v-toolbar-title @click="toTimeLine" class="on-click">My HandBook</v-toolbar-title>
      <v-spacer></v-spacer>
    </v-app-bar>
    <v-navigation-drawer v-model="drawer" fixed temporary>
      ...
      <v-row v-show="isAuthenticated" class="py-5">
        ...
        <v-col cols="4" class="d-flex align-center">
          <span class="on-click" @click="$router.push('/setting')"><v-icon>mdi-cog</v-icon></span>
        </v-col>
      </v-row>
      <v-divider></v-divider>
      <v-list>
        <v-list-item-group>
          ...
          <v-list-item v-for="(menu, i) in menuBar" :key="i" class="py-5" @click="$router.push(menu.url)" v-show="isAuthenticated">
  ...
  data () {
    return {
      ...
      menuBar: [
        ...
        {title: 'ブックマーク', url: '/bookmark', icon: 'bookmark-multiple-outline'},
      ],
    }
  },
  beforeCreate () {
