<template>
  <v-btn elevation="0" @click="googleLogin">
    <span class="blue--text text--darken-2">G</span
    ><span class="red--text text-darken-2">o</span
    ><span class="yellow--text text-darken-2">o</span
    ><span class="blue--text text--darken-2">g</span
    ><span class="green--text text--darken-2">l</span
    ><span class="red--text text--darken-2">e</span>でログイン</v-btn
  >
</template>
  ...
  methods: {
    ...
    async checkGoogleUser(user) {
      await this.$fb
        .firestore()
        .collection("users")
        .doc(user.uid).get().then(userData => {
        if (!userData.exists) {
