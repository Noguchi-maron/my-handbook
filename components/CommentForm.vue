                  ...
                  <v-col cols="10" md="8">
                    ...
                    <div>
                      <p class="d-flex justify-space-between">
                        ...
                        ><span
                          class="red--text on-click"
                          @click="deleteComment(comment, i)"
                          v-if="comment.deleteBtn"
                          >[ 削除 ]</span
                        >
                      </p>
                    </div>
          ...
          <v-form class="pt-5">
            <v-row justify="center">
              <v-col cols="10" md="8">
                ...
                <v-row justify="end">
                  <v-btn @click="submit" color="primary" elevation="0"
                    >送信</v-btn
                  >
                </v-row>
              </v-col>
...
export default {
  ...
  mounted() {
    this.setDeleteBtn();
    this.collection = this.$fb
      .firestore()
      .collection("users")
      .doc(this.bookData.authorId)
      .collection("books")
      .doc(this.bookData.id)
      .collection("comments");
  },
  computed: {
    ...mapGetters(["user"])
