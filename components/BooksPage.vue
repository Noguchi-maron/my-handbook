<template>
  <v-row justify="center" class="page-wrap" align="center" v-resize="resizeWindow">
    <v-col cols="11" md="10">
      <v-row class="align-center">
      <h3  class="d-inline-block">{{ bookData.title }}</h3>
      <v-icon class="ml-4 on-click" @click="toEditBook" v-if="!authorId">mdi-pencil</v-icon>
      </v-row>
      <v-row justify="space-between">
        <v-col cols="12" sm="6" class="pr-4">
          <div v-if="!mapActive" :style="{height: mapHeight + 'px'}"></div>
          <MapParts :destLists="dests" v-if="mapActive" @clickMarker="openInfo"></MapParts>
        </v-col>
        <v-col cols="12" sm="6" class="pl-4">
          <v-sheet>
            <v-chip-group v-model="selectDate" @change="getDests" mandatory active-class="primary--text" next-icon="mdi-chevron-right" :show-arrows="isIconShow">
              <v-chip v-for="date in dates"  :key="date">{{ date }}日目</v-chip>
            </v-chip-group>
      </v-sheet>
            <v-card outlined :style="{height: listHeight + 'px'}">
                  ...
                  <v-list>
                    <v-list-item-group>
                      <draggable v-model="dests" handle=".item-handle">
                        <v-list-item v-for="(dest, i) in dests" :key="dest.order" @click="openInfo(i)">
                          <v-icon color="#c0c0c0" v-show="isEdit" class="item-handle">mdi-drag-horizontal-variant</v-icon>
                          <v-list-item-title class="mr-4"><span v-show="!isEdit">{{ i+1 }}.</span> 　{{ dest.name }}</v-list-item-title>
                          <v-list-item-subtitle class="address" v-show="isShowAddress">{{ dest.address }}</v-list-item-subtitle>
                          <v-btn color="primary" text @click="editDestsBtn(dest.id)">詳細</v-btn>
                          <v-btn color="error" text v-show="isEdit" @click="deleteBtn(dest.id, i)">削除</v-btn>
                        </v-list-item>
                      </draggable>
                    </v-list-item-group>
                  </v-list>
...
export default {
  ...
  mounted () {
    ...
    this.book = this.$fb.firestore().collection('users').doc(this.userId).collection('books').doc(this.bookId)
    this.getBookData()
  },
  ...
  methods: {
    ...
    async getDests () {
      ...
      await this.book.collection(`dests`).where('date', '==', selectDate).orderBy('order').get().then((destData) => {
        if (destData) {
    ...
    async toSearch() {
      ...
      this.$router.push(`/mypage/${this.bookId}/search?date=${this.selectDate}&order=${this.dests.length}`)
    },
    //しおりの編集画面
    toEditBook () {
    ...
    async setOrder (newOrder, id) {
      await this.book.collection('dests').doc(id).update({order: newOrder})
    },
    //目的地の削除
    async deleteBtn (id, i) {
      await this.book.collection('dests').doc(id).delete()
    ...
    getDates () {
      ...
      const date1 = new Date(date1Data.year, date1Data.month - 1, date1Data.date)
      const date2 = new Date(date2Data.year, date2Data.month - 1, date2Data.date)
    ...
    resizeWindow () {
      if (window.innerWidth < 600) {
        ...
        this.listHeight = this.mapHeight * 2 - chipHeight - titleHeight - 50
      } else {
        this.listHeight = 450
      }
